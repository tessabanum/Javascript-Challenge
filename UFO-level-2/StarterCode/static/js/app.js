...
function createTableRow(rowData) {
	...
	tableRow.setAttribute("data-filter-datetime", rowData.datetime);
	tableRow.setAttribute("data-filter-city", rowData.city);
	tableRow.setAttribute("data-filter-state", rowData.state);
	tableRow.setAttribute("data-filter-country", rowData.country);
	tableRow.setAttribute("data-filter-shape", rowData.shape);
...
document.getElementById("filter-btn").addEventListener("click", function(e){
	...
	if (dateFilter) {
		...
		for (var i = 0; i < allRows.length; i ++) {
			if (allRows[i].getAttribute("data-filter-" + filterType) !== dateFilter) {
